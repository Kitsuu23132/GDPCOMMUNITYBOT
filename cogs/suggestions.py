...
class Suggestions(commands.Cog, name="Sugestii"):
    ...
    @app_commands.command(name="suggest", description="Trimite o sugestie pentru server")
    @app_commands.describe(content="Sugestia ta (minim 20 caractere)")
    async def suggest_cmd(self, interaction: discord.Interaction, content: str):
        if len(content) < 20:
            return await interaction.response.send_message(
                embed=error_embed("Sugestia trebuie să aibă cel puțin 20 de caractere."), ephemeral=True
            )
        await interaction.response.defer(ephemeral=True)
        sug_id = await db.create_suggestion(
            interaction.guild.id, interaction.user.id, content, now_iso()
        )
        settings = await db.get_guild_settings(interaction.guild.id)
        ch_id = settings.get("suggestion_channel") or 0
        ...
        if not ch:
            return await interaction.followup.send(
                embed=error_embed("Canal de sugestii neconfigurat."), ephemeral=True
            )
        sug_embed = build_suggestion_embed(sug_id, interaction.user, content, "pending", 0, 0)
        view = SuggestionVoteView(sug_id)
        msg = await ch.send(embed=sug_embed, view=view)
        await db.set_suggestion_message(sug_id, msg.id, ch.id)
        await interaction.followup.send(
            embed=success_embed(f"Sugestia **#{sug_id}** a fost trimisă în {ch.mention}!"),
            ephemeral=True
        )

    @app_commands.command(name="accept", description="[Staff] Acceptă o sugestie")
    @app_commands.describe(suggestion_id="ID-ul sugestiei", response="Răspunsul tău")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def accept_suggestion(self, interaction: discord.Interaction,
                                 suggestion_id: int, response: str = "Acceptat de staff."):
        await self._handle_suggestion(interaction, suggestion_id, "accepted", response)

    @app_commands.command(name="deny", description="[Staff] Respinge o sugestie")
    @app_commands.describe(suggestion_id="ID-ul sugestiei", response="Motivul respingerii")
    @app_commands.checks.has_permissions(manage_guild=True)
    async def deny_suggestion(self, interaction: discord.Interaction,
                               suggestion_id: int, response: str = "Respins de staff."):
        await self._handle_suggestion(interaction, suggestion_id, "denied", response)
