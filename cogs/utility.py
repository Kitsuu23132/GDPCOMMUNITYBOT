    ...
    async def help_cmd(self, interaction: discord.Interaction):
        e = embed(
            ...
            fields=[
                ...
                ("💰 Economie", "`/balance` `/daily` `/work` `/transfer` `/rob` `/shop` `/buy` `/coinflip`", False),
                ("⭐ Nivele", "`/rank` `/leaderboard` `/richest`", False),
                ("🎉 Fun", "`/8ball` `/dice` `/rps` `/guess` `/trivia` `/meme` `/joke` `/poll`", False),
                ...
                ("⚙️ Admin", "`/setwelcome` `/setgoodbye` `/setlog` `/setlevel` `/setprefix`", False),
            ]
        )
